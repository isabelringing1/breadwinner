...
function DailyOrder(props) {
    ...
    useEffect(() => {
        if (loaded){
            ...
            if (!dailyOrderUnlocked()){
                document.getElementById("do-bookmark-div").style.transform = "translateY(20vh)";
            }
        }   
    }, [loaded]);
    ...
    const updateDailyOrder = () => {
        ...
        if (dailyOrderNextRefreshTime == null){
            ...
            nextRefreshTime = new Date(refreshTimeToday) >= now ? refreshTimeToday : refreshTimeToday + 86400000; //+24 hours
    ...
    */
    ...
    const canClaim = () => {
        ...
        for (var i = 1; i < dailyOrderArray.length; i++){
    ...
    const tryClaimReward = (e) => {
        ...
        setClaimState("claimed")
        var timeSinceGeneration = Date.now() - (dailyOrderNextRefreshTime - 86400000)
        ...
        emitEvent("daily-order-claim", [totalDailyOrders + 1, totalTimely], null);
        e.stopPropagation();
    }
    ...
    var animate_in = () => {
      ...
      var jiggle_bookmark = () => {
        if (animating.current || showDailyOrder) return;
        document.getElementById("do-bookmark-body").classList.add("jiggle-bookmark");
        setTimeout(() => {
            document.getElementById("do-bookmark-body").classList.remove("jiggle-bookmark");
        }, 250);
      };
    ...
    const animateReward = (amount) => {
        ...
        var boundingBox = document.getElementById("daily-order-claim-button").getBoundingClientRect();
    ...
    var peek_in = () => {
        document.getElementById("do-bookmark-div").classList.add("peek-in-bookmark");
        setTimeout(() => {
          document
    ...
    return (<div id="daily-order-container" onClick={() => animate_out()}>
        <span id="do-reward-anim">100</span>

        <div id="do-bookmark-div">
            <img id="do-bookmark-body" className="bookmark-img" src={doBookmark} onClick={() => animate_in()} onMouseEnter={() => jiggle_bookmark()} />
        </div>
        <div id="daily-order">
            <div id="do-spirals">
            {Array.from(Array(15), (e, i) => {
                return <img src={spiral} key={i} className="do-spiral" />;
            })}
            </div>

            <div id="daily-order-content">
                <div id="daily-order-title"><img id="daily-order-title-img" src={title}/></div>
                <div id="daily-order-cards">
                    {
                        dailyOrderArray.map((order, i) => {
                            if (i == 0) return null;
                            return <DailyOrderCard order={order} i={i} key={i}/>
                        })
                    }
                </div>
                {dailyOrderArray[0] == -1 ? 
                    ...
                    : 
                    <div id="daily-order-reward-info">
                        Reward: <BCSymbol color="black" height={"3vh"}/>{formatNumber(dailyOrderArray[0])}
                    </div>
                }
